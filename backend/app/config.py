...
class Settings(BaseSettings):
    ...
    # ─── Реферальная программа (миграция 0032) ───────────────────────────
    # Дни полного доступа за приглашение. Схема односторонняя: их получает
    # ТОЛЬКО пригласивший, за каждого приведённого нового человека.
    # REFERRAL_DAYS_INVITED=0 — приглашённому дни не начисляются; значение
    # оставлено параметром, чтобы вернуть двустороннюю схему одной
    # переменной в .env, без правки кода.
    ...
    # Меняя значение, поправь и тексты: в боте они склоняются от этой
    # переменной, а в мини-аппе и на лендингах зашиты (TRIAL_DAYS в
    # miniapp/src/trial.ts) — иначе обещание разъедется с правами.
    ...
    # Пара VAPID-ключей: ими подписываются запросы к пуш-сервису браузера,
    # чтобы он знал, что отправитель тот же, что и при подписке.
    # Сгенерировать: python -m py_vapid --gen (см. docs/pwa-push.md).
    # Публичный ключ отдаётся клиенту, приватный не покидает сервер.
    # Пустые значения = механизм выключен, всё остальное работает как раньше.
