...
async def cmd_start(message: Message, command: CommandObject) -> None:
    ...
    # Экран «итог занятия» в мини-аппе уводит сюда: ?start=reminder и
    # ?start=invite. Без этих веток человек попадал бы на приветствие.
    if payload == "invite":
        await _send_invite_card(message, message.from_user)
        return
    if payload == "reminder":
        await cmd_reminder(message)
        return
    if payload == "guide":
...
async def _send_welcome(message: Message) -> None:
    ...
    # Приветствие ведёт в разговор, а не в инструкцию. Раньше первым делом
    # человека отправляли читать /guide — это откладывало первую фразу
    # по-английски, ради которой он и пришёл. Гайд теперь приходит
    # отдельным сообщением на следующий день после первой сессии.
    await message.answer(
        text=(
            f"Привет, {user_name}! 👋\n\n"
            ...
            "хоть «hello, how are you». Пять минут, и станет понятно, твоё это "
            "или нет."
        ),
        parse_mode="HTML",
        reply_markup=_miniapp_keyboard(),
...
async def _set_bot_commands() -> None:
    ...
    Не критично для работы: список хранится на стороне Telegram и меняется
    раз в полгода. Поэтому ошибка здесь не должна ронять бота — см. вызов
    в main().
    """
...
async def main() -> None:
    ...
    # Меню команд — вещь необязательная, и падать из-за него нельзя.
    # Доступ к Telegram с сервера периодически пропадает (провайдерская
    # фильтрация), и раньше единственный неудачный запрос здесь убивал
    # процесс ДО старта long polling: контейнер уходил в цикл рестартов
    # каждые тридцать секунд и не поднимался, даже когда сеть возвращалась.
    # Теперь бот стартует в любом случае, а polling переподключается сам.
    try:
        await _set_bot_commands()
    except Exception as exc:
        logger.warning("Меню команд не обновлено (%r) — продолжаем без него", exc)
